...
function manyPeople(count: number): Person[] {
    return Array.from({length: count}, (_, i) => makePerson({id: String(i), name: `Person ${i}`}));
}
...
describe('People page birth date column', () => {
    it('labels the birth date of a person who has one', () => {
        renderPage(ssrPage([makePerson({name: 'Anne Hathaway'})]));

        const born = new Date('1969-11-04').toLocaleDateString(undefined, {
            year: 'numeric',
            month: 'short',
            day: 'numeric'
        });
        expect(rowText('Anne Hathaway')).toContain(`Born ${born}`);
    });
